...
export default function MobileMenu({ mobileMenuOpen, setMobileMenuOpen }) {
  ...
  const isDashboardItem = (href) => {
    return href?.includes("/dashboard") || href?.startsWith("/db-");
  };
            ...
            {menuData.map((elm, i) => (
              <li key={i} className='menuNav__item -has-submenu js-has-submenu'>
                ...
                <ul
                  ...
                  {elm.submenu.map((elm2, i2) => (
                    <li key={i2} className=''>
                      {elm.label.toLowerCase() === "dashboard" ||
                      isDashboardItem(elm2.href) ? (
                        <a
                          onClick={(e) => handleDashboardClick(e, elm2.href)}
                          className={
                            pathname.split("/")[1] == elm2.href?.split("/")[1]
                              ? "activeMenu"
                              : ""
