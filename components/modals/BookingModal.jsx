...
export default function BookingModal({
  ...
  const tourData = {
    id: tour?.id || tour?.tourId || 39,
    title: tour?.title || "Tour Booking",
    basePrice: tour?.price || tour?.basePrice || 15000, // Base price for 1-4 people
    duration: tour?.duration || "4 hours",
  };

  // Calculate pricing based on transfer pricing structure
  const calculatePricing = () => {
    ...
    if (totalPeople <= 4) {
      // Same price for 1-4 people
      totalAmount = tourData.basePrice;
      priceBreakdown.push({
        description: `Group (${totalPeople} ${
          totalPeople === 1 ? "person" : "people"
        })`,
        quantity: 1,
        rate: tourData.basePrice,
        amount: tourData.basePrice,
      });
    } else {
      // Base price for first 4 people + individual pricing for extras
      const baseCost = tourData.basePrice;
      const extraPeople = totalPeople - 4;
      const extraCost = extraPeople * (tourData.basePrice / 4); // Individual rate based on base price divided by 4
  ...
  const createBooking = async (paymentData) => {
    try {
      ...
      const bookingPayload = {
        ...
        additionalServices:
          bookingData.isExtraService || bookingData.isServicePerPerson || false,
        totalAmount: pricing.totalAmount, // Use calculated amount
        currency: "USD", // Specify currency
        paymentData: {
          method: "paypal",
          transactionId: paymentData.transactionId,
            ...
            createOrder: (data, actions) => {
              return actions.order.create({
                purchase_units: [
                  {
                    amount: {
                      value: totalAmount.toFixed(2), // Use calculated amount
                      currency_code: "USD",
                    },
                    description: `${tourData.title} - ${formatDateForDisplay(
                      bookingData.selectedDate
